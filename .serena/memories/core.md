...
- Public invariants: exactly eight transform IDs; commands are direct (no `run`/`transform` wrapper); `toc tui` is explicit; one shared registry/pipeline; preserve CLI byte output, errors and exit codes.
