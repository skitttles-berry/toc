...
이 문서는 `docs/prd/init-prd.md`와 `docs/prd/v0.2-prd.md`의 후속 승인 설계다. 기능 구현과 단위 시험은 반영되었고 macOS·Linux 셸, 실제 TUI와 클립보드 통합 검증은 수행 대기 상태다. 해당 검증까지 완료되면 TUI에 한해 기존의 반응형 레이아웃, 고정 200밀리초 지연, 중간 결과 제외, 비 UTF-8 디코딩 결과 실패와 원시 이진 성공 결과 제외 규약을 대체한다. CLI 관련 규약은 대체하지 않는다. 전환 완료 전 규약의 기준은 계속 기존 PRD다.
...
이 절의 macOS·Linux 실제 환경 검증은 수행 대기 상태이며, 완료 판정과 기존 PRD 우선순위 전환은 해당 결과를 기록한 뒤에만 한다.
