...
이 문서는 `docs/prd/init-prd.md`와 `docs/prd/v0.2-prd.md`의 후속 승인 설계다. 구현이 완료되면 TUI에 한해 기존의 반응형 레이아웃, 고정 200밀리초 지연, 중간 결과 제외, 비 UTF-8 디코딩 결과 실패와 원시 이진 성공 결과 제외 규약을 대체한다. CLI 관련 규약은 대체하지 않는다. 구현 완료 전 실제 제품 동작은 계속 기존 PRD를 따른다.
...
Pipeline, Input, Output을 한 번에 하나씩 표시한다. `Tab`과 `Shift+Tab`으로 패널을 전환하며 현재 패널 이름을 탭과 Context Bar에 텍스트로 표시한다.
...
Text는 유효한 UTF-8 결과만 표시한다. UTF-8 검증이 실패하면 첫 실패 바이트 오프셋과 `Switch to Hex view` 안내를 표시하고 손실 대체 문자를 만들지 않는다.
...
01    Base64 Decode      4 B      1 B      6 µs     OK
02    Hex Encode         1 B      1 B      —        OFF
```
...
UTF-8 결과는 원본 문자열 전체를 복사한다. 기존 위험한 제어 문자 확인 절차를 유지한다. 비 UTF-8 결과는 공백 없는 소문자 Hex 문자열로 복사하고 `Copied as Hex`를 표시한다. Trace와 실행 오류에는 복사를 허용하지 않는다.

복사 형식은 현재 View가 아니라 원본 Artifact의 UTF-8 유효성으로 결정한다. 따라서 Text를 수동 고정한 상태에서 바이너리 결과가 나오더라도 Context Bar는 `Copy as Hex`를 표시한다. Hex 문자열의 두 배 길이는 검사된 산술로 계산하고 할당이나 운영체제 클립보드 작업이 실패하면 기존과 같은 비파괴 화면 오류로 처리한다.
