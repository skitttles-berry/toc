...
활성 상태와 실행 상태 뒤에 변환 이름을 표시한다. `OK`, `OFF`, `ERROR`,
`RUNNING`, `NOT RUN`과 `CANCELLED` 상태 단어는 Pipeline 행에서
중복해 표시하지 않는다.

너비가 부족하면 단계 크기 등 낮은 우선순위 정보부터 숨기되 `[ON]`,
...
사용한다. Modal은 현재 레이아웃 위에 기존 화면 → Dim → 오른쪽·아래 한 셀
Shadow → Clear → Modal 순서로 표시하며, 애니메이션은 사용하지 않는다.
...
입력이나 복사 내용을 로그와 오류 메시지에 포함하지 않는다.
...
* 복사 실패는 상태 메시지 외의 Input, Pipeline과 Output 결과를 변경하지
  않는다.
