...
- Context7 `/websites/rs_serde_json`: `arbitrary_precision`, 사용자 정의 방문자, 오류 줄·열
...
Use a Serde seed that walks every value but never builds a `Value`. It tracks decoded object keys with `HashSet<String>`. Do not count visitor maps as depth because `arbitrary_precision` numbers are exposed through an internal single-entry map. The locked `serde_json` recursion limit validates the 128-container boundary, and the lexical scanner in the next step repeats the limit as a defensive invariant.
...
fn validate(input: &[u8]) -> Result<(), TransformError> {
    ...
    result.map_err(|error| {
        ...
        let kind = if message.contains(DUPLICATE_KEY_MARKER) {
