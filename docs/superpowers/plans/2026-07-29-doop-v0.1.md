...
- Context7 `/ratatui/ratatui-textarea`: 커서·선택 스타일 수정자
...
아래 작업별 코드 조각은 TDD 시점의 중간 상태다. 최종 구현에서는 `TransformDefinition.accepts_binary`와 `behavior`가 입력 조건과 고정 동작 도움말의 단일 진실 공급원이며, `InvalidUtf8Output`은 64바이트 16진수 미리보기와 전체 바이트 수를 함께 보존한다. TTY 출력 거부 오류는 안전한 제한 길이 미리보기를 가지며, `NO_COLOR`의 Input 커서와 선택 영역은 색상 없이 `REVERSED` 수정자를 사용한다.
...
- Produces: `TransformDefinition { id, display_name, description, behavior, accepts_binary, apply }`
