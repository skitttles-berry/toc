...
표시합니다. Ready 상태의 활성 Artifact 크기는 제목 공간이 충분할 때만 덧붙입니다.
...
2026-08-02 `codex/tui-shortcuts-output`에서 형식, 경고 금지 잠금 Clippy,
rustdoc, 잠금 패키징과 임시 경로 잠금 설치를 실행했다. 전체 시험 결과는
라이브러리 263개 통과·3개 무시, 실행 파일 0개, CLI 통합 15개 통과로
합계 278개 통과·3개 무시였고 실패는 없었다. 설치본은 `toc 0.2.0`이었다.
release redraw 측정의 무조건 다시 그리기 최솟값·중앙값·최댓값은
`67.913042ms`·`68.660125ms`·`69.533084ms`, 변경 시 다시 그리기는
`124.542µs`·`141.166µs`·`172.917µs`였고 실제 다시 그리기는 1회였다.
최대 입력 편집은 `2.804125ms`·`2.956333ms`·`3.060166ms`, 64 MiB UTF-8
판정은 `2.22825ms`·`2.302166ms`·`2.432ms`였다. 시간값은 성공 기준이 아니다.
