...
2026-08-02 `codex/tui-responsiveness-navigation`에서 형식, 경고 금지 잠금
Clippy, rustdoc, 작업 트리 잠금 패키징과 임시 경로 오프라인 잠금 설치를
실행했다. 전체 시험 결과는 라이브러리 284개 통과·3개 무시, 실행 파일 0개,
CLI 통합 15개 통과로 합계 299개 통과·3개 무시였고 실패는 없었다. 설치본은
`toc 0.2.0`이었다. release redraw 측정의 무조건 다시 그리기
최솟값·중앙값·최댓값은 `76.020917ms`·`76.970958ms`·`78.118125ms`, 변경 시
다시 그리기는 `142.959µs`·`152.666µs`·`210.667µs`였고 실제 다시 그리기는
1회였다. 최대 입력 편집은 `2.856833ms`·`2.94675ms`·`3.238166ms`, 64 MiB
UTF-8 판정은 `2.223042ms`·`2.347375ms`·`2.663458ms`였다. 시간값은 성공
기준이 아니다.
...
확인했다. 검증 환경은 Darwin이므로 X11·Wayland 경로는 실행하지 않았다.
